 ...
 *
 * 表示中の一覧を汚さないように、計算中は recommend:{user}:progress にだけ
 * 作業内容を保存し、全分類が終わったタイミングで一気に
 * recommend:{user}:display にコピーする。
 * ------------------------------------------------------------
 */
...
async function fetchInvidious(path) {
  const instances = shuffle(INVIDIOUS_INSTANCES).slice(0, TRY_INSTANCES_LIMIT);
...
module.exports = async function handler(req, res) {
  try {
    ...
    switch (progress.stage) {
      ...
      case "watch_recent": {
        await Promise.allSettled(
          progress.recentWatch.map(async (item) => {
            const id = item?.id || (typeof item === "string" ? item : "");
            if (!id) return;
      ...
      case "watch_mid": {
        await Promise.allSettled(
          progress.midWatch.map(async (item) => {
            const id = item?.id || (typeof item === "string" ? item : "");
            if (!id) return;
      ...
      case "long_tag": {
        await Promise.allSettled(
          progress.longWatch.map(async (item) => {
            const id = item?.id || (typeof item === "string" ? item : "");
            if (!id) return;
        ...
        if (progress.candidates.length < 90) {
          ...
          await Promise.allSettled(
            triggers.map(async (v) => {
              if (progress.candidates.length >= 90) return;
